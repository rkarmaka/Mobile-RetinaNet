...
def cell_density(img,plot=True):
    ...
    num=len(cont)
    if num>10:
        cd=np.round((num*446*304*10)/(np.sum(imbw/255.)*1.23),0)
        area=[cv.contourArea(cont[i]) for i in range(len(cont))]
        cov=np.round(np.std(area)*100/np.mean(area),2)
        hex_=np.round(hexa(imbw,plot),2)
    else:
        cd=0
        cov=0
        hex_=0
