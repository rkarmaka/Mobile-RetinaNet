...
########################################################################################################################
# Mobile-CellNet
def mobile_vesselNet(img_dim, pre_trained=False):
    f = [8, 16, 32, 64, 128]

    if pre_trained:
        mobileNetv2 = MobileNetV2(input_shape=(img_dim, img_dim, 3), weights='imagenet', include_top=False)
        ...
        x = Conv2D(f[0], (3, 3), strides=1, activation=None, padding='same')(inp)
        x = BatchNormalization()(x)
        x = Activation(tf.nn.relu)(x)
        x = DepthwiseConv2D(kernel_size=(3, 3), strides=1, activation=None, padding='same')(x)
        x = BatchNormalization()(x)
        x = Activation(tf.nn.relu)(x)
        x = Conv2D(f[0], (3, 3), strides=1, activation=None, padding='same')(x)
        x = BatchNormalization()(x)

        # first bottleneck layer
        c1 = func.mobile_bottleneck(x, 6 * f[0], f[0], strides=2)
        c1 = func.mobile_bottleneck(c1, 6 * f[0], f[0], strides=1)
        c1 = func.mobile_bottleneck(c1, 6 * f[0], f[0], strides=1)

        # second bottleneck layer
        c2 = func.mobile_bottleneck(c1, 6 * f[0], f[0], strides=2)
        c2 = func.mobile_bottleneck(c2, 6 * f[0], f[0], strides=1)
        c2 = func.mobile_bottleneck(c2, 6 * f[0], f[0], strides=1)

        # third bottleneck layer
        c3 = func.mobile_bottleneck(c2, 6 * f[1], f[1], strides=2)
        c3 = func.mobile_bottleneck(c3, 6 * f[1], f[1], strides=1)
        c3 = func.mobile_bottleneck(c3, 6 * f[1], f[1], strides=1)

        # forth bottleneck layer
        c4 = func.mobile_bottleneck(c3, 6 * f[2], f[2], strides=2)
        c4 = func.mobile_bottleneck(c4, 6 * f[2], f[2], strides=1)
        c4 = func.mobile_bottleneck(c4, 6 * f[2], f[2], strides=1)

        # fifth bottleneck layer
        c5 = func.mobile_bottleneck(c4, 6 * f[2], f[2], strides=2)
        c5 = func.mobile_bottleneck(c5, 6 * f[2], f[2], strides=1)
        c5 = func.mobile_bottleneck(c5, 6 * f[2], f[2], strides=1)

    e4 = Conv2DTranspose(f[2], kernel_size=3, strides=2, padding='same')(c5)
    e4 = Concatenate()([e4, c4])
    e4 = func.mobile_bottleneck(e4, 6 * f[2], f[2], add=False)
    e4 = func.mobile_bottleneck(e4, 6 * f[2], f[2], strides=1)

    e3 = Conv2DTranspose(f[1], kernel_size=3, strides=2, padding='same')(e4)
    e3 = Concatenate()([e3, c3])
    e3 = func.mobile_bottleneck(e3, 6 * f[1], f[1], add=False)
    e3 = func.mobile_bottleneck(e3, 6 * f[1], f[1], strides=1)

    # upsampling 2
    e2 = Conv2DTranspose(f[0], kernel_size=3, strides=2, padding='same')(e3)
    e2 = Concatenate()([e2, c2])
    e2 = func.mobile_bottleneck(e2, 6 * f[0], f[0], add=False)
    e2 = func.mobile_bottleneck(e2, 6 * f[0], f[0], strides=1)

    # e2=Conv2D(32,kernel_size=3,strides=1,padding='same')(e2)

    e1 = Conv2DTranspose(f[0], kernel_size=3, strides=2, padding='same')(e2)
    e1 = Concatenate()([e1, c1])
    e1 = func.mobile_bottleneck(e1, 6 * f[0], f[0], add=False)
    e1 = func.mobile_bottleneck(e1, 6 * f[0], f[0], strides=1)

    # upsampling 1
    e0 = Conv2DTranspose(f[0], kernel_size=3, strides=2, padding='same')(e1)
    # e0=Concatenate()([e0,x])
    e0 = func.mobile_bottleneck(e0, 6 * f[0], f[0], add=False)
    e0 = func.mobile_bottleneck(e0, 6 * f[0], f[0], strides=1)

    out = Conv2D(f[0], 3, strides=1, padding='same')(e0)
    out = BatchNormalization()(out)
    out = Activation(tf.nn.relu)(out)
    out = Conv2D(f[0], 3, strides=1, padding='same')(out)
    out = BatchNormalization()(out)
    out = Activation(tf.nn.relu)(out)
